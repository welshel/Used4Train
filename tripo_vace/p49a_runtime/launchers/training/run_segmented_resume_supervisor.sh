...
gc_config="$train_dir/P49A_SELECTIVE_GC_ACTIVE_CONFIG.json"
gc_tail_blocks="${P49A_GC_TAIL_BLOCKS:-6}"
rm -f "$final_status"
if [[ ! -s "$audit" ]]; then
...
write_gc_config() {
  printf '{\n  "updated_at": "%s",\n  "checkpointing_period": 1,\n  "uncheckpointed_tail_blocks_per_chain": %s,\n  "fallback_order": [6, 4, 0],\n  "formal_training_world_size": 4,\n  "status": "active"\n}\n' \
    "$(date -Is)" "$gc_tail_blocks" > "$gc_config"
}
...
while true; do
  ...
  started=$(date -Is)
  log="$train_dir/train_4gpu_resume${start}_gc_tail${gc_tail_blocks}.log"
  set +e
  "$PWD/envs/p49a_vace13b/bin/accelerate" launch --multi_gpu --num_processes 4 --mixed_precision no --main_process_port 0 scripts/p49a_train_resume.py \
    ...
    --resume_note "Same P49A fresh LoRA continuation from complete global step ${start}; optimizer reinitialized; scheduler restored to global step ${start}; selective GC tail ${gc_tail_blocks}; no old LoRA." \
    > "$log" 2>&1
  status=$?
  set -e
  finished=$(date -Is)
  ...
  grep -q 'CUDA out of memory' "$log" && termination="cuda_oom_gc_tail${gc_tail_blocks}"
  printf '%s,%s,%s,%s,%s,%s\n' "$started" "$start" "$finished" "$status" "$latest" "$termination" >> "$audit"
  if (( latest >= 700 )); then
    printf '0\n' > "$final_status"
    exit 0
  fi
  if [[ "$termination" == cuda_oom_gc_tail* ]]; then
    if (( gc_tail_blocks > 4 )); then
      gc_tail_blocks=4
    elif (( gc_tail_blocks > 0 )); then
      gc_tail_blocks=0
    else
      printf '%s\n' "$status" > "$final_status"
      exit "$status"
