...
def export_renderer_transformer_hf_checkpoint(checkpoint_path: str, hf_path: str, global_step: int) -> None:
    """Export just the trained high-noise transformer after a completed DCP save.

    The official renderer training entry saves resumable DCP checkpoints, while
    the official renderer inference path consumes safetensors.  On torch 2.7
    VeOmni's official DCP-to-state-dict converter is the compatible route.  We
    deliberately export only ``diff_dec.transformer``: the frozen T5 and VAE
    remain in the fixed base snapshot, and a validation worker must never need
    or receive clean target data as an inference condition.
    """
    if dist.get_rank() != 0:
        return

    transformer_prefix = "diff_dec.transformer."
    full_state = ckpt_to_state_dict(checkpoint_path, ckpt_manager="dcp")
    transformer_state = {
        name: tensor
    ...
    if not transformer_state:
        raise RuntimeError(
            f"DCP checkpoint {checkpoint_path} did not contain any {transformer_prefix} tensors"
        )

    temp_path = f"{hf_path}.tmp"
    if os.path.exists(temp_path):
        raise RuntimeError(f"refusing to overwrite incomplete HF export path: {temp_path}")
    save_model_weights(
        temp_path,
        transformer_state,
        save_dtype="bfloat16",
    ...
    os.replace(temp_path, hf_path)
    completion = {
        "global_step": global_step,
        "checkpoint_path": checkpoint_path,
        "export": "official_veomni_dcp_to_safetensors",
        "tensor_prefix": transformer_prefix,
        "tensor_count": len(transformer_state),
        "clean_used_as_inference_input": False,
    }
    marker_path = os.path.join(hf_path, "P51A_HF_EXPORT_COMPLETE.json")
    ...
    logger.info_rank0(
        "Exported %d high-noise transformer tensors for validation: %s",
        len(transformer_state),
        hf_path,
    )
...
def main():
    ...
    if args.train.checkpoint.load_path:
        ...
        # P51A's finite 72-row iterable source reports an exhausted cursor when
        # VeOmni restores its serialized dataloader state in a new short-lived
        # process.  Keep the model/optimizer/scheduler/RNG continuation but
        # deliberately recreate the replenishable iterator instead.  Its epoch
        # seed is tied to the trusted global step below, so each 20-step process
        # gets a deterministic, distinct traversal rather than replaying an
        # invalid exhausted cursor or warm-starting another task.
        resume_data_epoch = global_step
        environ_meter.load_state_dict(state["extra_state"]["environ_meter"])
        torch.set_rng_state(state["extra_state"]["torch_rng_state"])
        if args.train.global_rank == 0:
            resume_record = {
                "status": "complete",
                "resumed_from": args.train.checkpoint.load_path,
                "global_step": global_step,
                "optimizer_restored": True,
                "lr_scheduler_restored": True,
                "torch_rng_restored": True,
                "dataloader_state_restored": False,
                "dataloader_policy": "recreated_finite_iterable_with_set_epoch(global_step)",
                "fresh_run": True,
                "clean_used_as_inference_input": False,
            }
            resume_record_path = os.path.join(
                args.train.checkpoint.output_dir,
                f"P51A_RESUME_EVENT_step{global_step:04d}.json",
            )
            resume_record_tmp = f"{resume_record_path}.tmp"
            with open(resume_record_tmp, "w", encoding="utf-8") as f:
                json.dump(resume_record, f, indent=2, sort_keys=True)
                f.write("\n")
            os.replace(resume_record_tmp, resume_record_path)
            logger.info_rank0(
                "P51A resumed step %d with a recreated finite dataloader iterator.",
                global_step,
            )
        dist.barrier()
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        if hasattr(train_dataloader, "set_epoch"):
            train_dataloader.set_epoch(
                resume_data_epoch if resume_data_epoch is not None and epoch == start_epoch else epoch
            )
        ...
        for _ in range(start_step, args.train_steps):
            ...
            if args.train.checkpoint.save_steps and global_step % args.train.checkpoint.save_steps == 0:
                ...
                if (
                    args.train.checkpoint.save_hf_weights
                    and args.train.checkpoint.hf_save_steps
                    and global_step % args.train.checkpoint.hf_save_steps == 0
                ):
                    hf_path = os.path.join(save_path, "hf_ckpt")
                    export_renderer_transformer_hf_checkpoint(save_path, hf_path, global_step)
                # ``checkpointer.save`` is synchronous for P51A.  Publish a
                # root-level completion marker only after its optional HF
                # export has also finished.  The durable controller treats a
                # checkpoint without this marker as interrupted and archives
                # it instead of attempting an unsafe resume from it.
                if args.train.global_rank == 0:
                    completion = {
                        "status": "complete",
                        "global_step": global_step,
                        "checkpoint_manager": args.train.checkpoint.manager,
                        "save_async": bool(args.train.checkpoint.save_async),
                        "hf_export_expected": bool(
                            args.train.checkpoint.save_hf_weights
                            and args.train.checkpoint.hf_save_steps
                            and global_step % args.train.checkpoint.hf_save_steps == 0
                        ),
                    ...
                    marker_path = os.path.join(save_path, "P51A_DCP_COMPLETE.json")
                    marker_tmp = f"{marker_path}.tmp"
                    with open(marker_tmp, "w", encoding="utf-8") as f:
                        json.dump(completion, f, indent=2, sort_keys=True)
                        f.write("\n")
                    os.replace(marker_tmp, marker_path)
                    logger.info_rank0("Published complete DCP checkpoint marker: %s", marker_path)
                dist.barrier()
        pbar.close()
        start_step = 0
    synchronize()
