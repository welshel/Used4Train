...
def _get_sharding_plan(
    ...
    for key, tensor_meta in metadata.state_dict_metadata.items():
        ...
        if hf_key:
            if prefix and not hf_key.startswith(prefix):
                continue
...
def save_model_weights(
    ...
    if prefix:
        logger.info(f"Filtering keys with prefix: '{prefix}'")
    shards, total_size, all_dcp_keys = _get_sharding_plan(checkpoint_path, shard_size, save_dtype, prefix)
...
def merge_to_hf_pt(
    load_dir: str, save_path: str, model_assets_dir: Optional[str] = None, shard_size: int = 2_000_000_000, save_dtype: Optional[Union[str, torch.dtype]] = "bfloat16", prefix: Optional[str] = None
) -> None:
    ...
    save_model_weights(save_path, load_dir, shard_size=shard_size, model_assets=model_assets, save_dtype=save_dtype, prefix=prefix)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Dtype to save weights (default: bfloat16)",
    )
    parser.add_argument(
        "--prefix",
        type=str,
        default=None,
        help="Only save weights starting with this prefix (applied to HuggingFace keys)",
    )
    ...
    merge_to_hf_pt(load_dir, save_dir, model_assets_dir, shard_size=shard_size, save_dtype=args.save_dtype, prefix=args.prefix)
