...
ROOT_OUT="$P39/04_quality_metrics/quick_start31"
...
run_validation() {
  ...
  "$PY" "$VALIDATE" \
    ...
    --output-dir "$out" --label "$label" --lora "$lora" \
    --only-starts 31 --base-seed 1337 --num-inference-steps 20 \
    --cfg-scale 5.0 --vace-scale 1.0 --fps 12 > "$out/inference.log" 2>&1
}
...
run_validation \
  h672_950 \
  "$P39/03_672_training/H672_step900_to950/run/checkpoints/optimizer_step-0950.safetensors" \
  "$ROOT_OUT/h672_950"
