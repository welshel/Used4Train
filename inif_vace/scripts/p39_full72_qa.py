...
def validate_video_contract(
    ...
    height: int,
    expected_fps: int,
    expected_frames: int = 72,
) -> None:
    expected_shape = (expected_frames, height, width, 3)
    if video.shape != expected_shape or abs(fps - expected_fps) > 1e-6:
...
def make_keyframes(videos: dict[str, np.ndarray], path: Path) -> None:
    rows = []
    names = {"condition": "Condition", "output": "P3.9 Candidate", "clean": "Clean QA"}
    ...
    cv2.putText(header, "P3.9 Full72 keyframes | raw output", (18, 39), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (25, 25, 25), 2, cv2.LINE_AA)
...
def make_loop_board(videos: dict[str, np.ndarray], path: Path) -> None:
    ...
    cv2.putText(header, "P3.9 loop seam | F66-F71 then F00-F05", (18, 37), cv2.FONT_HERSHEY_SIMPLEX, 0.80, (25, 25, 25), 2, cv2.LINE_AA)
    write_rgb(path, np.concatenate([header, board], axis=0))
...
def make_comparison_video(
    ...
    names = {"condition": "Condition", "output": "P3.9 Candidate RAW", "clean": "Clean QA"}
...
def main() -> None:
    ...
    clean_path = args.output_dir / "P39_FULL72_CLEAN_QA.mp4"
    build_clean_video(pair_rows, clean_path, (args.width, args.height))
    ...
    (args.output_dir / "P39_FULL72_METRICS.json").write_text(json.dumps(result, indent=2) + "\n")
    with (args.output_dir / "P39_FULL72_TRANSITIONS.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(transitions[0]))
        writer.writeheader()
        writer.writerows(transitions)

    make_keyframes(videos, args.output_dir / "P39_FULL72_KEYFRAMES.png")
    make_loop_board(videos, args.output_dir / "P39_LOOP_SEAM_BOARD.png")
    make_comparison_video(videos, args.output_dir / "P39_FULL72_COMPARISON.mp4", list(range(72)), args.fps)
    make_comparison_video(
        videos,
        args.output_dir / "P39_LOOP_SEAM_PREVIEW.mp4",
