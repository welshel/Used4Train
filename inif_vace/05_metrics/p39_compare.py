...
def validate_video_contract(
    ...
    if video.shape != expected or abs(fps - 12.0) > 1e-6:
        raise ValueError(
            f"video contract failed: shape={video.shape} fps={fps}; expected={expected} fps=12"
        )


def load_historical_metrics(path: Path) -> dict:
    payload = json.loads(path.read_text())
    if "historical_l33_800_512" in payload:
        return payload["historical_l33_800_512"]
    if "aggregate" in payload and "l33_800" in payload["aggregate"]:
        return payload["aggregate"]["l33_800"]
    raise ValueError(f"unsupported historical metrics format: {path}")


def parse_starts(value: str) -> tuple[int, ...]:
    try:
        starts = tuple(int(item.strip()) for item in value.split(",") if item.strip())
    except ValueError as error:
        raise ValueError(f"validation starts must be integers from {STARTS}: {value}") from error
    if not starts or len(set(starts)) != len(starts) or any(start not in STARTS for start in starts):
        raise ValueError(f"validation starts must be a unique subset of {STARTS}: {value}")
    return starts


def resolution_comparison_title(
    baseline_display: str,
    candidate_display: str,
    width: int,
) -> str:
    return f"{baseline_display} vs {candidate_display} | matched inference{int(width)}"
...
def main() -> None:
    ...
    parser.add_argument("--baseline-display", default="P3.8 H640@100")
    parser.add_argument("--candidate-display", default="P3.9 H672")
    parser.add_argument("--starts", default=",".join(str(start) for start in STARTS))
    args = parser.parse_args()
    starts = parse_starts(args.starts)
    DISPLAY["baseline"] = args.baseline_display
    DISPLAY["candidate"] = args.candidate_display
    ...
    for start in starts:
        ...
        for label, path in paths.items():
            ...
            validate_video_contract(video, fps, args.width, args.height)
            videos[label] = video
        loaded[start] = videos
        ...
        write_comparison_video(videos, videos_out / f"P39_START{start:02d}_COMPARISON.mp4")

    summary = aggregate(rows)
    historical = load_historical_metrics(args.historical_metrics)
    ...
    (args.output_dir / "P39_METRICS_SUMMARY.json").write_text(json.dumps(result, indent=2) + "\n")
    write_csv(args.output_dir / "P39_ALL_METRICS.csv", rows)
    write_csv(args.output_dir / "HIGH_FREQUENCY_REGION_METRICS.csv", hf_rows)

    start = 31 if 31 in loaded else starts[0]
    start_title = f"start{start:02d}"
    make_keyframe_board(loaded[start], boards / "P39_SHARPNESS_BOARD.png", f"P3.9 sharpness | {start_title}")
    make_keyframe_board(
        loaded[start],
        boards / "P39_RESOLUTION_COMPARISON_BOARD.png",
        resolution_comparison_title(DISPLAY["baseline"], DISPLAY["candidate"], args.width),
    )
    make_track_board(loaded[start], boards / "P39_COMPLEX_OBJECT_BOARD.png", f"P3.9 complex-object persistence | {start_title}", boxes[start])
    crop_y = int(round(args.height * 220 / 640))
    make_track_board(loaded[start], boards / "P39_CURTAIN_PLANT_TABLE_BOARD.png", f"P3.9 curtain / plant / table track | {start_title}", (0, crop_y, args.width, args.height))
    make_track_board(loaded[start], boards / "P39_TEMPORAL_PERSISTENCE_BOARD.png", f"P3.9 temporal persistence | {start_title}", (0, 0, args.width, args.height))
