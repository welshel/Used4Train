...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a clean-leakage-free P3.8 full RoomTour")
...
def main() -> None:
    args = parse_args()
    if (args.width, args.height) != (640, 640):
        raise ValueError("current promoted P3.8 full72 inference contract is 640x640")
    ...
    condition_path = args.output_dir / "P38_FULL72_CONDITION.mp4"
    ...
    if args.method == "direct73":
        ...
        save_video(output73, str(args.output_dir / "P38_FULL73_DIRECT_DEBUG.mp4"), fps=args.fps, quality=5)
        generated_frames = canonicalize_rotated_frames(list(output73[:72]), args.start_index)
        generated_path = args.output_dir / "P38_FULL72_DIRECT_RAW.mp4"
        ...
        generated_path = args.output_dir / "P38_FULL72_OVERLAP33_RAW.mp4"
        save_video(generated_frames, str(generated_path), fps=args.fps, quality=5)
    ...
    (args.output_dir / f"P38_FULL72_{args.method.upper()}_SUMMARY.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    print("P38_FULL72_SUMMARY " + json.dumps(summary), flush=True)
